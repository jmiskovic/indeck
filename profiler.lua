...
----------------------------------------------------------------------------
-- Modified for inDECK to return string instead of writing to file.
-- Usage:
--   p = require('profiler')
--   p.start('p=-s')
--   --[[ hot code ]]
--   profile_report = p.stop()
--
----------------------------------------------------------------------------
...
local iostring = setmetatable({}, {
  write = function(self, str) table.insert(self, str) end,
  readAll = function(self)
    local report = table.concat(self)
    for i,_ in ipairs(self) do self[i] = nil end  -- reset
    return report
  end,
  })
getmetatable(iostring).__index = getmetatable(iostring)
